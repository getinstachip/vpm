...
use std::fs;
use std::path::Path;
use elasticsearch::{Elasticsearch, indices::IndicesCreateParts, Error as ElasticsearchError, http::transport::{TransportBuilder, SingleNodeConnectionPool}, IndexParts};
use elasticsearch::http::headers::{HeaderMap, HeaderValue, HeaderName, CONTENT_LENGTH};
...
// pub const ES_API_KEY: &str = "essu_YTIxNU9VTTFSVUphWVZwM1NscEhWbEZQYld3NmIzZExhVmhDUlc5VVZDMXZUVGxIVUVkR1JrVXpRUT09AAAAALJJ010=";
...
pub(crate) fn create_client() -> Result<Elasticsearch, Box<dyn std::error::Error>> {
    ...
    headers.insert(AUTHORIZATION, HeaderValue::from_str(&format!("ApiKey {}", ES_API_KEY)).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?);
...
async fn create_document(fpath: &Path) -> Result<HashMap<String, Value>, Box<dyn std::error::Error>> {
    let module_code = fs::read_to_string(fpath)?;
    let embedding = generate_embedding(&module_code).await?;
    Ok(HashMap::from([
        ("name".to_string(), Value::String(fpath.file_name().unwrap().to_str().unwrap().to_string())),
        ("code".to_string(), Value::String(module_code)),
        ("embedding".to_string(), json!(embedding)),
    ]))
}

pub(crate) async fn embed_library(fpath: &Path, index: &str) -> Result<Vec<Value>, Box<dyn std::error::Error>> {
...
pub(crate) async fn create_index(client: &Elasticsearch, index_name: &str) -> Result<(), ElasticsearchError> {
...
pub(crate) async fn insert_documents(es_client: &Elasticsearch, index_name: &str, embedded_documents: &Vec<Value>) -> Result<(), ElasticsearchError> {
    ...
    for chunk in embedded_documents.chunks(2) {
        if chunk.len() == 2 {
            ...
            headers.insert(AUTHORIZATION, HeaderValue::from_str(&format!("ApiKey {}", ES_API_KEY)).unwrap());
...
pub(crate) async fn vector_search(client: &Elasticsearch, index_name: &str, query_vector: Vec<f32>, top_k: usize) -> Result<Vec<HashMap<String, Value>>, ElasticsearchError> {
    ...
    let response = client.search(elasticsearch::SearchParts::Index(&[index_name]))
        .body(search_query)
        .send()
        .await?;
