...
impl Execute for Docs {
    async fn execute(&self) -> Result<()> {
        if let Some(url) = &self.url {
            let content = fetch_module_content(&self.module, url).await?;
            generate_docs(&self.module, &content, None).await?;
        } else {
            ...
            match find_and_read_module(&vpm_modules_dir, module_name) {
                Some((content, module_path)) => {
                    ...
                    generate_docs(module_name, &content, Some(module_path)).await?;
                },
